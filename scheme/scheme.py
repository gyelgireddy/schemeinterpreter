...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        check_procedure(value)
        return value.eval_call(expr.second, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def define(self, symbol, value):
        ...
        # BEGIN PROBLEM 3
        self.value = value
        self.bindings[symbol] = self.value
        # END PROBLEM 3
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        args = operands.map(lambda operands: scheme_eval(operands, env))
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        #value = Thunk(operands, env)
        #args = operands.map(lambda operands: complete_eval(value))
        #operands = operands.second
        #value = self.apply(operands, env)
        value =scheme_apply(self, operands, env)
        return scheme_eval(complete_eval(value), env)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            #result = scheme_apply(value, args, env)
            result = value.eval_call(rest, env)
            # END PROBLEM 20
    return result
